...
class blk(gr.sync_block):  # other base classes are basic_block, decim_block, interp_block
    ...
    def __init__(self, sampling_frequency=6e5, file_path='/home/froztgal/example_channel.mat'):  # only default arguments here
        """arguments to this function show up as parameters in GRC"""
        gr.sync_block.__init__(
            self,
            name='Channel_block_AChS',   # will show up in GRC
            in_sig=[np.complex64],
        ...
        self.upsamp = math.ceil(self.fc / (self.df * self.hsamp)) * self.hsamp
        self.buffer = deque([0] * self.upsamp)
        self.counters_limits = [self.dt * self.fc, self.h.shape[1]]
    ...
    def read_file(self, h_file):
        ...
        self.df = float(mat['df'][0][0])
        self.dt = float(mat['dt'][0][0])
        self.h = mat['hmat']
